...
| `09_fastlog_record` | `tl.record`/`fastlog`, `Recording`, `to_trace`, `dry_run`, `repredicate`, `halt`, `save=` (the removed `keep_op=`/`keep_module=` aliases raise `TypeError`) | yes | `dry_run` is `save=`-only (module-predicate dry runs removed); `repredicate` still spells its overrides `other_keep_op=`/`other_keep_module=`; `Recording.__repr__` is an ~85 KB event dump; no "record everything" spelling |
