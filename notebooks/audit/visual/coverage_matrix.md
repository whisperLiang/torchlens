...
| `theme:colorblind` | vis_theme='colorblind' | p53 |
| `theme:high_contrast` | vis_theme='high_contrast' | p53 |
...
| `code_panel:true` | code_panel=True source panel | p48 |
| `code_panel:init_forward` | code_panel='init+forward' source panel | p48 |
| `font_size` | font_size= override | p49 |
| `dpi` | dpi= render density override | p49 |
| `for_paper` | for_paper=True publication styling | p49 |
...
| `intervention:node_mark` | vis_intervention_mode='node_mark' | p62 |
| `intervention:as_node` | vis_intervention_mode='as_node' hook nodes | p62 |
| `cone:on` | vis_show_cone=True downstream cone styling | p63 |
| `cone:off` | vis_show_cone=False | p63 |
| `input_transform_summary` | show_input_transform_summary=True input xlabel | p51 |
| `raw_io_render` | save_raw_input/save_raw_output thumbnails on boundary nodes | p50 |
| `draw_backward:rolled` | draw_backward default rolled grad_fn graph | p55 |
| `draw_backward:unrolled` | draw_backward vis_mode='unrolled' | p55 |
| `draw_backward:bwd` | draw_backward bwd= pass selection | p56 |
| `draw_combined` | draw_combined forward+backward graph | p57 |
| `combined:intervening_cluster` | draw_combined intervening_cluster placement | p58 |
| `backward_node_spec_fn` | draw_combined backward-half styling callback | p57 |
| `overlay:intervention` | node_overlay='intervention' fired-site borders | p45 |
| `code_panel:fallback` | code_panel embedded-cluster fallback (rank engine / non-composable format) | p48 |
| `batch_render` | batch_render thumbnail policy (auto/all/first/first_n/shape_only) | p50 |
| `raw_output_render` | decoded-output label grammar on the output node | p50 |
| `surface:fastlog_preview` | Trace.preview_fastlog predicate-decision coloring | p71 |
| `surface:bundle_diff` | tl.viz.bundle_diff paired clean-vs-intervened delta render | p72 |
...
| `node:intervention` | intervention site/hook rendering | p62 |
| `node:grad_fn` | backward grad_fn nodes | p55 |
...
| `edge:backward` | backward gradient edges | p55, p57 |
| `edge:fwd_bwd_tie` | dashed forward-to-backward correspondence edges | p57 |
...
| `label:arm` | conditional IF/ELIF/ELSE arm labels | p60 |
| `control_flow:branch` | tensor-driven if/else taken-arm rendering | p29, p60, p61 |
| `topology:parallel` | parallel branch / merge topology | p5, p11, p67 |
| `topology:transformer` | transformer / attention block topology | p66 |
| `scale:real_model` | real torchvision-scale architecture | p32, p37, p65 |
| `degenerate` | degenerate graphs (single op, no modules, scalar out) | p69 |
...
| 47 | F | `f5b_checked_suppression` | Checked suppression: redundant constructor args (default-on) | `small_conv` | 2 |
| 48 | F | `f6_code_panel` | code_panel: source code alongside the graph | `demo_model` | 3 |
| 49 | F | `f7_typography` | Typography: font_size, dpi, for_paper | `tiny_mlp` | 3 |
| 50 | F | `f8_raw_io` | Raw input thumbnails and decoded-output rows on boundary nodes | `small_conv` | 3 |
| 51 | F | `f9_input_transform` | show_input_transform_summary: preprocessing provenance | `tiny_mlp` | 1 |
| 53 | G | `g1_themes` | vis_theme: torchlens / paper / dark / colorblind / high_contrast | `tiny_mlp` | 5 |
| 55 | H | `h1_backward` | draw_backward: the grad_fn graph | `linear_relu` | 2 |
| 56 | H | `h2_backward_opts` | draw_backward options: direction and pass selection | `linear_relu` | 2 |
| 57 | H | `h3_combined` | draw_combined: forward and backward in one graph | `linear_relu`, `scalar_out` | 3 |
| 58 | H | `h4_combined_cluster` | draw_combined: intervening_cluster placement | `linear_relu` | 2 |
| 60 | I | `i1_conditionals` | if/else and elif ladders: taken-arm rendering | `elif_ladder`, `simple_if_else` | 2 |
| 61 | I | `i2_branch_cnn` | Conditional dual-head CNN | `tiny_branch_cnn` | 1 |
| 62 | I | `i3_intervention_modes` | vis_intervention_mode: 'node_mark' vs 'as_node' | `tiny_mlp` | 2 |
| 63 | I | `i4_cone` | vis_show_cone: the downstream affected region | `tiny_mlp` | 2 |
| 65 | J | `j1_resnet_overview` | ResNet-18 at collapse='auto': the intended overview experience | `resnet18` | 1 |
| 66 | J | `j2_transformer` | Transformer encoder, fully unrolled | `tiny_transformer` | 1 |
| 67 | J | `j3_inception` | Inception-style branching: parallel paths and concat merges | `mini_inception` | 1 |
