...
def _grep_for_pattern(pattern: str, paths: list[str]) -> list[str]:
    ...
    try:
        ...
        return _grep_for_pattern_python(pattern, paths)
    if result.returncode not in {0, 1}:
        raise RuntimeError(result.stderr)
    return [line for line in result.stdout.splitlines() if line]


def _grep_for_pattern_python(pattern: str, paths: list[str]) -> list[str]:
    """Search repository paths with a Python fallback when ``rg`` is unavailable."""

    repo_root = _repo_root()
    regex = re.compile(pattern)
    matches: list[str] = []
    text_suffixes = {".py", ".md", ".rst", ".txt", ".json", ".toml", ".yaml", ".yml"}
