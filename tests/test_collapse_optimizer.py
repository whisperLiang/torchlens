...
def test_bert_and_distilbert_small_config_auto_cuts_stay_pinned() -> None:
    ...
    transformers = _require_transformers()
    # The 5.x attention/embedding refactor changes the executed graph. Both
    # supported majors are exercised, with their own reviewed structural pins.
    transformers_major = int(transformers.__version__.split(".")[0])
    expected_bert_count, expected_distil_count = {4: (23, 18), 5: (21, 21)}[transformers_major]
    ...
    try:
        ...
        assert bert_result.visible_count == expected_bert_count
        assert bert_result.selected == {
            "encoder.layer.0",
            "encoder.layer.1",
        }
        assert _landmark_swallow_count(bert_trace, bert_result) == 0
    finally:
        bert_trace.cleanup()
    ...
    try:
        ...
        assert distil_result.visible_count == expected_distil_count
        expected_selected = {
        ...
        if transformers_major == 4:
            expected_selected.add("embeddings")
        assert distil_result.selected == expected_selected
        assert _landmark_swallow_count(distil_trace, distil_result) == 0
    finally:
