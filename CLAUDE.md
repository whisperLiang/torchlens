...
boundary-noise grace, charged on min(wall, cpu)) — gc-collection pauses charge a session
ledger, not the test they land in — an offender fails the session it ran in. `pytest -n auto` requires the
