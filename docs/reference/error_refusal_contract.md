...
| `param_substitution_occurrence_underivable` | A PARAM-selection `do()` could not address every consumption of the parameter: no matching top-level template component, a nested container position, a released/legacy capture without identity or barcode, or a multi-pass (recurrence-grouped) consumer — a named v1 engine limitation for recurrently reused params such as tied weights (spelling DOCUMENTED-UNSTABLE) | Target a parameter whose consumers are single-pass top-level arguments, or re-capture live with `intervention_ready=True` |
...
| `shard_local_persistence_unsupported` | Ordinary save of a trace carrying the shard-local capture marker (`distributed_scope == "rank_local_shard"`) while the active schema does not persist the disclosure — the PERMANENT erasure-prevention invariant at the bundle-save chokepoint: a shard-local trace must never reload as a plain trace (L8/F6; spelling DOCUMENTED-UNSTABLE) | Analyze in-session; persistence lands with the coordinated schema bump that flips the registered marker fields |
