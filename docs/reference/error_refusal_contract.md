...
| `extraction_resume_signature_mismatch` | A resumed extraction's run parameters (layers, batch size, transform disclosure, stimulus descriptor, or stimulus count) differ from the artifact's recorded signature (spelling DOCUMENTED-UNSTABLE) | Re-run with the artifact's original configuration, or delete the output directory to start fresh |
