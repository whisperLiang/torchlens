...
```
import torchlens
  |- exposes 98 top-level public names in __all__
...
| Path | Purpose |
|------|---------|
| `__init__.py` | Top-level API, 98-name `__all__`, deprecation shims, `peek`/`extract` helpers |
...
- `merged/` - cross-rank merging (C1): `tl.merge_ranks`/`tl.merge_report`, the
  ...
  artifact (routes 2 of the 98 `__all__` names; own AGENTS.md).
